...
import useFetch from "../../hooks/hebergeurfetcheatt";
...
const DatataHebergeur= ({ columns }) => {
 ...
 useEffect(() => {
  ...
  const handleUpdate = async (updatedData) => {
    ...
    try {
      await axios.put(`/${path}/${id}`, { isAdmin: true });
      setList((prevList) =>
        prevList.map((item) =>
          item._id === id ? { ...item, isAdmin: true } : item
        )
      );
    } catch (err) {
      ...
      renderCell: (params) => {
        return (
          <div className="cellAction">
            ...
            <div className="editButton" onClick={() => handleApprove(params.row._id)}>Approve</div>
          </div>
        );
      },
    },

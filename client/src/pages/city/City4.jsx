...
const City = () => {
  ...
  const city = location.state?.city || 'maison d hotes';
  const { data, loading } = useFetch(`/hotels?type=maison&min=${min || 0}&max=${max || 999}&minD=${minD || 0}&maxD=${maxD || 999}&amenities=${amenities.join(',')}`);
